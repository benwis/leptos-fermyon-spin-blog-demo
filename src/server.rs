...
#[http_component]
async fn handle_leptos_blog_demo_finished(req: IncomingRequest, resp_out: ResponseOutparam) {
    let mut conf = leptos::get_configuration(None).await.unwrap();
    conf.leptos_options.output_name = "leptos_blog_demo_finished".to_owned();

    // Register server functions
    register_explicit::<crate::pages::home::GetCount>();
    register_explicit::<crate::pages::home::UpdateCount>();
    register_explicit::<crate::functions::blog::AddPost>();
    register_explicit::<crate::functions::blog::GetPost>();
